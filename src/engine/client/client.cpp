...
{
	const int CmdLen = 196; // Keep buffer big, for ClRunOnJoin
	char aCmdBufs[4][CmdLen] = {};
	int NumCmds = 0;
	...
	if(m_ServerCapabilities.m_ChatHighBandwidth)
	{
		str_format(aCmdBufs[NumCmds++], CmdLen, "highbandwidth %d", g_Config.m_ClHighBandwidth);
	}
	if(m_ServerCapabilities.m_ChatEmote && (g_Config.m_ClDummyDefaultEyes || g_Config.m_ClPlayerDefaultEyes))
	{
...
static CServerCapabilities GetServerCapabilities(int Version, int Flags, bool Sixup)
{
	...
	Result.m_ChatHighBandwidth = false;
	Result.m_ChatEmote = false;
	Result.m_ChatRunOnJoin = false;
	if(Version >= 1)
	{
	...
	if(Version >= 6)
	{
		Result.m_ChatHighBandwidth = Flags & SERVERCAPFLAG_CHATHIGHBANDWIDTH;
