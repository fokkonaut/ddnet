...
void CProjectile::Tick()
{
	...
	{
		vec2 Core;
		if(Collision()->MoverSpeed(CurPos.x, CurPos.y, &Core))
		{
			m_StartTick = GameWorld()->GameTick();
			m_Direction = normalize(Core);

			// Snap the axis perpendicular to travel to the tile's centerline
			int Tx = std::clamp((int)CurPos.x / 32, 0, Collision()->GetWidth() - 1);
			int Ty = std::clamp((int)CurPos.y / 32, 0, Collision()->GetHeight() - 1);
