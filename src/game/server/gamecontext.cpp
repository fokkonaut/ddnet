...
void CGameContext::WhisperId(int ClientId, int VictimId, const char *pMessage)
{
	...
	if(Server()->IsSixup(ClientId) || GetClientVersion(ClientId) >= VERSION_DDNET_WHISPER)
	{
		...
		Server()->SendPackMsgTranslateWhisper(&Msg, MSGFLAG_VITAL | MSGFLAG_NORECORD, ClientId);
	}
	else
	{
	...
	if(Server()->IsSixup(VictimId) || GetClientVersion(VictimId) >= VERSION_DDNET_WHISPER)
	{
		...
		Server()->SendPackMsgTranslateWhisper(&Msg, MSGFLAG_VITAL | MSGFLAG_NORECORD, VictimId);
	}
	else
	{
...
{
	// Rejoin=true when calling InitPlayerMap because that will make sure each disconnect packet gets sent out to 0.7 clients correctly before inserting
