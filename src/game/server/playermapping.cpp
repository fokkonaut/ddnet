	...
	if(ClientId == -1)
	{
		...
		for(auto &Map : m_aMap)
		{
			...
			if(Overhang != Map.m_TotalOverhang)
			{
				...
				m_aMap[ClientId].m_NumPages = maximum(1, (Overhang + (int)CPlayerMap::ESeeOthers::MAX_NUM_SEE_OTHERS - 1) / (int)CPlayerMap::ESeeOthers::MAX_NUM_SEE_OTHERS);
...
const char *CPlayerMapping::GetSeeOthersName(int ClientId, char (&aName)[MAX_NAME_LENGTH]) const
{
	...
	{
		str_format(aName, sizeof(aName), "⋅ %d/%d | Close", Page, Page);
	}
	else if (State != CPlayerMap::ESeeOthers::STATE_NONE)
	{
		if(m_aMap[ClientId].m_TotalOverhang > (int)CPlayerMap::ESeeOthers::MAX_NUM_SEE_OTHERS)
			str_format(aName, sizeof(aName), "⋅ %d/%d", Page, m_aMap[ClientId].m_NumPages);
		else
